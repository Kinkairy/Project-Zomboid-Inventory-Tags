...
test('default selects nothing, never all',function()
 local p,c=pane({item('Food')});eq(S.rule(p,c).all,false);eq(S.apply(p,c,S.rule(p,c)),0)
end)
test('whole parent uses exact shared grouping including slash forms',function()
 ...
 eq(#selectedItems(p),2);eq(S.rule(p,c).categories.Material,true);eq(S.rule(p,c).categories.ToolWeapon,nil)
end)
test('leaf click is immediate and a second category accumulates',function()
 local p,c=pane({item('Food'),item('FirstAid'),item('Tool')})
 clicked(named(named(menu(p,c),IT.text('GroupFood')).child,C.label('Food')));eq(#selectedItems(p),1)
 clicked(named(named(menu(p,c),IT.text('GroupMedical')).child,C.label('FirstAid')));eq(#selectedItems(p),2)
 clicked(named(named(menu(p,c),IT.text('GroupFood')).child,C.label('Food')));eq(#selectedItems(p),1)
end)
...
test('choices isolated between panes players and containers',function()
 local p,c=pane({item('Food')});S.apply(p,c,rule({'Food'}))
 local p2,c2=pane({item('Tool')});p2.player=1;eq(S.rule(p2,c2).categories.Food,nil);eq(S.rule(p,c).categories.Food,true)
 eq(S.rule(p,c2).categories.Food,nil)
end)
...
test('new handler passes exact originating pane',function()
 local p,c=pane({item('Food')});local h=ihandlers[4]:new();h.inventoryWindow={inventoryPane=p};h.playerNum=0
